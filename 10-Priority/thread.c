...
/* Return to Thread mode from PSP which do not use FPU,
 * and execution use PSP after return. */
...
void __attribute__((naked)) pendsv_handler()
{
    ...
    int low_prio = lastTask;
    for (int i = 0; i < MAX_TASKS; ++i) {
        if (tasks[i].in_use) {
            if (*(tasks[i].prio) < *(tasks[low_prio].prio)) {
                low_prio = i;
            } /*else {
                (*(tasks[i].prio))--;
            }*/
    ...
    lastTask = low_prio;
    (*(tasks[lastTask].prio))++;
    if (*(tasks[lastTask].prio) > MAX_PRIORITY) {
        *(tasks[lastTask].prio) = 0;
    }

    /* Move the task's stack pointer address into r0 */
    asm volatile("mov r0, %0\n" : : "r"(tasks[low_prio].stack));
...
int thread_create(void (*run)(void *), void *userdata, int *prio)
{
    /* Find a free thing */
    int threadId = 0;
    uint32_t *stack;
